class PusherController < ApplicationController
  ...
  def auth
      if current_user && Room.includes(:rooms_users).where('rooms_users.user_id'=>current_user.id,'rooms.id'=>params[:room_id].to_i).exists? && (params[:channel_name] == "private-#{params[:room_id]}" || params[:channel_name] == "private-#{current_user.id}"|| params[:channel_name] == "presence-status")
