class MessageController < ApplicationController
  ...
  def new
    if  Room.where("id in (?)",RoomsUser.where(:user_id=>current_user.id).pluck(:room_id)).pluck(:id).include?(params[:room_id].to_i)
      ...
      Pusher['private-'+"#{params[:room_id]}"].trigger('new_message', {:room_id=>params[:room_id],:user_id=>current_user.id,:login=>current_user.login,:avatar=>avatar_url(current_user),:message=>with_link(message.body),:create_at=>message.created_at.strftime("%a %T")})
    end
  end
