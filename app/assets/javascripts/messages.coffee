  ...
  Handlebars.registerHelper "safe_mess", (messag) ->
    if messag.length > 240 or messag.match(/http.*((youtu.be\/)|(v\/)|(\/u\/\w\/)|(embed\/)|(watch\?))\??v?=?([^#\&\?].\S\S*)/) or messag.match(/http.*(jpg|gif|jpeg)/) or messag.match(/http:\/\/(coub\.com\/view\/.*|coub\.com\/embed\/.*)/i)
      if messag.match(/http.*((youtu.be\/)|(v\/)|(\/u\/\w\/)|(embed\/)|(watch\?))\??v?=?([^#\&\?].\S\S*)/) or messag.match(/http.*(jpg|gif|jpeg)/) or messag.match(/http:\/\/(coub\.com\/view\/.*|coub\.com\/embed\/.*)/i)
        "<div id=\"short-text\" style=\"display: block;\">" + "<small class=\"pull-right text-muted\">" + "<span class=\"glyphicon glyphicon-chevron-down\" style=\"cursor: pointer;\"></span></small>" + "<p class=\"primary-font\">" + "<div class=\"text-muted\">" + "<i>" + "this text has a content..." + "</i></div></p></div>" + "<div id=\"long-text\" style=\"display: none;\">" + "<small class=\"pull-right text-muted\">" + "<span class=\"glyphicon glyphicon-chevron-up\" style=\"cursor: pointer;\"></span></small>" + "<p>" + $.trim(changetags(safe_tags_replace(messag))) + "</p>" + "</div>"
      else
